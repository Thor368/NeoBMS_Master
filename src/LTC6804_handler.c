...
void BMS_RES_sets(uint32_t i)
{
	...
	chips[i].Status = RES;
	chips[i].last_CV = Tick;
	chips[i].last_TEST = chips[i].last_CV + 10000;
	chips[i].last_CV += 1000;
...
#ifdef BMS_I_SENSOR
	if ((BMS_I_SENSOR == i) && (BMS_temp_count[i] > 0))
		chips[i].Aux = BMS_AUX_Current_Temp;
	else if (BMS_I_SENSOR == i)
		chips[i].Aux = BMS_AUX_Current;
	else if (BMS_temp_count[i] > 0)
		chips[i].Aux = BMS_AUX_Temp;
	else
#endif
		chips[i].Aux = BMS_AUX_None;
...
{
	for (uint8_t i = 0; i < BMS_cell_count[index]; i++)
	{
		...
		else if (chips[index].Cell_U[i] >= BMS_OV)
		{
			if (!(chips[index].Cell_OV_timer[i]))
				chips[index].Cell_OV_timer[i] = Tick + BMS_OV_Delay;
			else if (chips[index].Cell_OV_timer[i] <= Tick)
			{
		...
		else if (chips[index].Cell_U[i] <= BMS_soft_UV)
		{
			if (!(chips[index].Cell_UV_timer[i]))
				chips[index].Cell_UV_timer[i] = Tick + BMS_UV_Delay;
			else if (chips[index].Cell_UV_timer[i] <= Tick)
			{
...
void BMS_Calc_Voltages(uint8_t index)
{
	...
	if (chips[index].Cell_Count != BMS_cell_count[index])
...
void BMS_Calc_Temp(uint32_t i)
{
	...
	for (uint8_t j = 0; j < BMS_temp_count[i]; j++)
	{
		...
		float T = LTC_get_AUX_raw(&(chips[i].chip), k) - chips[i].chip.STAR.VA/2;
		T = BMS_Temp_c2*T*T + BMS_Temp_c1*T + BMS_Temp_c0;
		chips[i].Temp_sensors[j] = round(T);
	}
}
#endif
...
void LTC_Balancing_handler()
{
	if (BMS_Balance_Timer <= Tick)
		BMS_Balance_Timer += 1000;
	else
		return;
...
/*			if (chips[i].Int_Temp > (BMS_ITMP_LIM + BMS_ITMP_HYST*2))
			...
			else
			{
				...
				if (chips[i].Int_Temp < (BMS_ITMP_LIM - BMS_ITMP_HYST/2))
				{
					if ((chips[i].Balance_timer >= 10) && (chips[i].Balance_derating < BMS_cell_count[i]))
					{
				...
				{
					if (chips[i].Balance_derating < BMS_cell_count[i])
					{
			...
			{
				uint8_t highest;
				for (uint8_t x = 0; x < (BMS_cell_count[i]+1); x++)
					if (!chips[i].Cell_Bleed[x])
						highest = x;
				if (highest > BMS_cell_count[i])
					break;

				for (uint8_t x = 0; x < BMS_cell_count[i]; x++)
...
void BMS_IO_handler()
{
	...
	for (uint8_t x = 0; x < BMS_chip_count; x++)
	{
		...
		for (uint8_t y = 0; y < BMS_cell_count[x]; y++)
		{
	...
	{
		BMS_enable_charge;
		BMS_charge_delay_timer = Tick + BMS_fault_delay;
	}
	else if (BMS_charge_delay_timer < Tick)
		BMS_disable_charge;
#endif
...
void LTC_handler()
{
	...
	for (uint8_t i = 0; i < BMS_chip_count; i++)
	{
		switch (chips[i].Status)
		{
				...
				if (LTC_Start(&(chips[i].chip), LTC_START_PLADC))
				{
						...
						(chips[i].Aux == BMS_AUX_Current))
					{
						if (Battery_I_offset == 0)
						{
							...
							{
								Battery_I_offset_timer = Tick + 10000;
								Battery_I_offset = (int32_t) chips[i].chip.AVAR.G1V*16;
							}
						}
						else if (Battery_I_offset_timer > Tick)
						{
			...
			case WAIT:
				if (chips[i].last_TEST <= Tick)
				{
					chips[i].last_TEST += 10000;
					
					chips[i].Status = TEST_MUX;
				}
				else if ((chips[i].last_CV) <= Tick)
				{
					chips[i].last_CV += 1000;
